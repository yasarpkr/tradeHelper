...
let test = async() => {
    ...
    while(true){
        ...
        //     await wait(1 * 1000 * 10 ) // wait last integer minute} 
            ...
            countX = 0;
            for(let element4 of upperList) {
                for(let elementOld of dataListOld){
                    if(element4.coin == elementOld.coin){

                        // Check ema negative of new candle
                        if (elementOld.ema12 < 0 && element4.ema12 > 0){
                            upperList[countX].breakCount ++

                        }   
                        if (elementOld.ema25 < 0 && element4.ema25 > 0){
                            upperList[countX].breakCount ++
                        }   
                        if (elementOld.ema50 < 0 && element4.ema50 > 0){
                            upperList[countX].breakCount ++
                        }
                    break;   
                    }
            ...
            for(let element of upperList){
                ...
                breakUpList[count] = {};
                breakUpList[count].coin = element.coin;
                breakUpList[count].rsi6 = element.rsi6;
                breakUpList[count].fifteenminVPower = element.fifteenminVPower;
                breakUpList[count].breakCount = element.breakCount;        
                count++;
                }
            }
            ...
            // linkListOfLunar = [];
            // count  = 0
            // for(let elements of mailListOf){
